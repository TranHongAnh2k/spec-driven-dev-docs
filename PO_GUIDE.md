...
```
PO/BA là người duy nhất viết và approve tài liệu đầu vào:

  PO/BA                     Dev team
  ──────────────────────    ──────────────────────────────
  /define-product           đọc product-definition
  /generate-prd        →    /generate-bdd (BE)
  /review-context           /generate-design-spec + BDD (FE/App)
  /refine-prd (SA review)
  /generate-design-spec →   BDD (FE/App)
```
...
- Thông báo dev team khi có PRD mới hoặc update

**PO/BA KHÔNG cần quan tâm:**
- BDD feature files — dev team tự generate
- Tech docs — dev team tự generate
- Source code — hoàn toàn dev team

### Tại sao PO/BA không cần quan tâm đến BDD?

BDD là **kết quả phái sinh từ PRD**, không phải nguồn gốc yêu cầu. Phân chia trách nhiệm:
...
**1. BDD được tự động sinh từ PRD**
`/generate-bdd` đọc PRD → tự viết Gherkin scenarios. PO đã hoàn thành trách nhiệm khi viết AC trong PRD — agent làm phần còn lại.

**2. BDD là ngôn ngữ kỹ thuật, không phải ngôn ngữ nghiệp vụ**

PO viết trong PRD:
> *"Sai password 5 lần → khoá tài khoản 30 phút"*

Dev/QA dịch thành BDD:
```gherkin
Given user calls POST /auth/login with invalid credentials 5 times
When the 5th attempt returns 401
Then account_locked = true AND locked_until = NOW + 30min
```

Chi tiết `POST /auth/login`, `account_locked`, `locked_until` là concern của dev/QA. PO không cần expertise để đánh giá điều này.

**3. Mỗi platform sinh BDD riêng từ cùng 1 PRD**

| Platform | BDD focus | Tool |
|---|---|---|
| Web | UI interactions (click button, assert toast) | Playwright |
| App | Mobile gestures (tap element, assert screen) | Detox/Maestro |
| BE | API contracts (HTTP request, assert response) | Jest/Supertest |

Cả 3 đều trace về **cùng 1 PRD**. Nếu PO phải review từng BDD variant → bottleneck không cần thiết.

**PO vẫn có trách nhiệm gián tiếp:** Khi BDD fail → dev trace về PRD → hỏi PO làm rõ AC. PO tham gia để **clarify yêu cầu nghiệp vụ**, không phải để viết/review Gherkin.
...
| `/generate-design-spec` | Tạo Design Spec cho FE/App | Sau khi PRD approved, trước khi FE/App generate BDD |
...
git commit -m "feat(auth): add FEAT-01 login PRD v1.0 [approved]"
git push
```

**Thông báo dev team:** "FEAT-01 PRD đã approved, domain: `auth`"
...
### Tình huống 3 — Tạo Design Spec sau khi PRD approved

**Bối cảnh:** PRD đã được approve. Cần tạo Design Spec cho FE web trước khi FE team generate BDD.
...
Agent sẽ hỏi:
- Platform: `web` hay `app`?
- Figma link (nếu có)
- Thông tin màn hình/luồng

Output: `specs/design-spec/auth/FEAT-01-design-spec-web.md`

**Sau khi Designer review:**
1. Mở Design Spec file
2. Cập nhật Figma links vào từng screen
3. Cập nhật `@trace.status: approved`

```bash
git add specs/design-spec/auth/FEAT-01-design-spec-web.md
git commit -m "feat(auth): add FEAT-01 web design spec [approved]"
git push
```

**Thông báo FE team:** "Design Spec FEAT-01 web đã ready, có thể generate BDD"

> **Lưu ý:** Nếu cần Design Spec cho App → chạy lại `/generate-design-spec` và chọn `app`  
> Output: `FEAT-01-design-spec-app.md` — file riêng, không ghi đè file web.
...
**Git:**
- [ ] Đã commit và push lên spec repo
- [ ] Thông báo dev team domain name để họ cập nhật `git submodule update --remote`
